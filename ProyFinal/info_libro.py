...
class info_libro(CTkToplevel):
    ...
    def __init__(self, parent, libro, *args, **kwargs):
        ...
        # Tamaño amplio para organizar todo de forma premium
    ...
    def inicializar_interfaz(self):
        ...
        # =====================================================================
        self.frame_izq = CTkFrame(self, fg_color="#bec1f6", border_color="#696dc2", border_width=1, corner_radius=5)
        ...
        # Botón Atrás
        self.btn_atras = CTkButton(self.frame_izq, text="← Volver al catálogo", fg_color="#1a1c43", text_color="white", command=self.destroy)
        self.btn_atras.grid(row=0, column=0, padx=20, pady=(20, 10), sticky="nw")

        # Contenedor de Metadatos
        self.frame_meta = CTkFrame(self.frame_izq, fg_color="transparent")
        ...
        self.lbl_titulo = CTkLabel(self.frame_meta, text=self.libro.titulo, text_color="black", font=("Calibri Light", 22, "bold"), wraplength=350, justify="left")
        ...
        for idx, (label, valor) in enumerate(metadatos):
            row_idx = idx + 1
            lbl_tag = CTkLabel(self.frame_meta, text=label, text_color="#1a1c43", font=("Calibri Light", 14, "bold"))
            ...
            lbl_val = CTkLabel(self.frame_meta, text=valor, text_color=color_val, font=("Calibri Light", 14, "bold" if label == "Estado:" else "normal"), wraplength=220, justify="left")
            lbl_val.grid(row=row_idx, column=1, padx=10, pady=8, sticky="w")

        # =====================================================================
        # SECCIÓN DE DATOS DE PRÉSTAMO ACTIVO
        # =====================================================================
        self.frame_prestamo_info = CTkFrame(self.frame_izq, fg_color="#dbddff", corner_radius=5)
        ...
        if prestamo_activo:
            self.lbl_pres_titulo = CTkLabel(self.frame_prestamo_info, text="Información del Préstamo", text_color="#1a1c43", font=("Calibri Light", 15, "bold"))
        ...
        # =====================================================================
        self.frame_botones = CTkFrame(self.frame_izq, fg_color="transparent")
        ...
        # =====================================================================
        self.frame_der = CTkFrame(self, fg_color="#bec1f6", border_color="#696dc2", border_width=1, corner_radius=5)
        ...
        self.frame_stats = CTkFrame(self.frame_der, fg_color="#dbddff", corner_radius=5)
        ...
        self.lbl_cant_pal = CTkLabel(self.box_palabras, text=f"{palabras_totales:,}", text_color="#1a1c43", font=("Calibri Light", 22, "bold"))
        self.lbl_cant_pal.pack(pady=(5, 0))
        self.lbl_lbl_pal = CTkLabel(self.box_palabras, text="Palabras Totales", text_color="black", font=("Calibri Light", 13))
        self.lbl_lbl_pal.pack(pady=(0, 5))

        self.box_paginas = CTkFrame(self.frame_stats, fg_color="transparent")
        self.box_paginas.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
        self.lbl_cant_pag = CTkLabel(self.box_paginas, text=str(paginas_estimadas), text_color="#1a1c43", font=("Calibri Light", 22, "bold"))
        self.lbl_cant_pag.pack(pady=(5, 0))
        self.lbl_lbl_pag = CTkLabel(self.box_paginas, text="Páginas Estimadas", text_color="black", font=("Calibri Light", 13))
        self.lbl_lbl_pag.pack(pady=(0, 5))

        # --- SECCIÓN 2: Vista previa (600 palabras) y Buscador interno ---
        self.frame_preview = CTkFrame(self.frame_der, fg_color="transparent")
        ...
        self.box_txt_preview = CTkFrame(self.frame_preview, fg_color="#dbddff", corner_radius=5)
        self.box_txt_preview.grid(row=0, column=0, padx=(0, 10), pady=0, sticky="nsew")
        
        self.lbl_prev_title = CTkLabel(self.box_txt_preview, text="Vista Previa del Libro (600 Palabras)", text_color="black", font=("Calibri Light", 14, "bold"))
        self.lbl_prev_title.pack(anchor="w", padx=15, pady=(10, 5))

        self.txt_preview = CTkTextbox(self.box_txt_preview, fg_color="white", text_color="black", font=("Calibri Light", 12), wrap="word", height=140)
        ...
        if preview_content.strip() == "":
            preview_content = "(El archivo de texto local no se encuentra o está vacío. Descárgalo para ver la vista previa.)"
        ...
        self.box_buscador = CTkFrame(self.frame_preview, fg_color="#dbddff", corner_radius=5)
        self.box_buscador.grid(row=0, column=1, padx=(10, 0), pady=0, sticky="nsew")
        self.box_buscador.grid_columnconfigure(0, weight=1)

        self.lbl_bus_title = CTkLabel(self.box_buscador, text="Buscador de Palabra", text_color="black", font=("Calibri Light", 14, "bold"))
        self.lbl_bus_title.grid(row=0, column=0, padx=15, pady=(10, 5), sticky="w")

        self.entry_palabra = CTkEntry(self.box_buscador, placeholder_text="Escribe una palabra...", fg_color="white", border_color="#1a1c43")
        self.entry_palabra.grid(row=1, column=0, padx=15, pady=5, sticky="we")

        self.btn_buscar_interna = CTkButton(self.box_buscador, text="Contar apariciones", fg_color="#1a1c43", hover_color="#272a63", command=self.contar_palabra_interna)
        self.btn_buscar_interna.grid(row=2, column=0, padx=15, pady=5, sticky="we")

        self.lbl_resultado_busqueda = CTkLabel(self.box_buscador, text="Conteo: -", text_color="#1a1c43", font=("Calibri Light", 15, "bold"))
        self.lbl_resultado_busqueda.grid(row=3, column=0, padx=15, pady=10, sticky="w")

        # --- SECCIÓN 3: Gráfica de palabras más repetidas ---
        self.frame_grafica = CTkFrame(self.frame_der, fg_color="#dbddff", corner_radius=5)
        ...
        self.lbl_graf_title = CTkLabel(self.frame_grafica, text="Palabras Más Frecuentes (Excluyendo Conectores)", text_color="black", font=("Calibri Light", 14, "bold"))
        self.lbl_graf_title.grid(row=0, column=0, padx=15, pady=(10, 5), sticky="w")

        # Contenedor de las barras
        self.box_bars = CTkScrollableFrame(self.frame_grafica, fg_color="transparent")
        ...
        if frecuentes:
            ...
            for i, (palabra, conteo) in enumerate(frecuentes):
                ...
                lbl_plb = CTkLabel(self.box_bars, text=f"{i+1}. {palabra}", text_color="black", font=("Calibri Light", 12, "bold"))
